...
Virtual Gallery - это персональная художественная галерея, разработанная для представления работ художницы Татьяны Дьяковой в онлайн-формате. Проект реализован с использованием Django и развернут с использованием Docker, Nginx и PostgreSQL.
...
Создайте файл`.env`по подобию `.env.example` в корне проекта и отредактируйте его:
...
DB_USER=postgres
DB_PASSWORD=your-strong-database-password
ALLOWED_HOSTS=yourdomain.com,www.yourdomain.com (опционально)
ADMIN_URL=your-secret-admin-path/
```
